...
`.gitkeep` files in every bind-mounted directory (`team-collections/dev/minio-data/`,
...
This repo pins the Supabase CLI as a dev dependency, so `pnpm install` at the repo root
provides it; run it as `pnpm supabase ...` (or put `node_modules/.bin` on your PATH so the
bare `supabase ...` commands below work). Deno 2.9+ is needed for `supabase functions
serve` and the edge-function tests: `winget install DenoLand.Deno`. See the repo root
README.md for the exact versions.

Verify: `pnpm supabase --version` and `deno --version`
...
```
team-collections/dev/
  docker-compose.yml        MinIO + init job
  functions.env             Local-only edge-function secrets (passed with --env-file)
  DEV-CREDENTIALS.md        How the functions hand out MinIO vs. real STS credentials
...
The dev seed users live in `supabase/seeds/tc_dev.sql` (wired into `[db.seed]` in
`supabase/config.toml`), and the auth settings the stack needs are already in
`supabase/config.toml`'s `[auth]` section.
...
`supabase/seeds/tc_dev.sql` is listed in `[db.seed].sql_paths`, so `supabase db reset`
(or `pnpm reset`) applies the migrations and then the seed. To load it into an already
running database without a reset:

```bash
supabase db query --file supabase/seeds/tc_dev.sql
```
...
```bash
docker compose -f team-collections/dev/docker-compose.yml up -d
```
...
```bash
supabase functions serve --env-file team-collections/dev/functions.env
# or: pnpm dev:functions:tc
```
...
`supabase_network_bloom-supabase-bloom-core`, external, created by `supabase start`) and
...
have `team-collections/dev/functions.env` — so every S3-touching function fails with
...
rm supabase_db_bloom-supabase-bloom-core`, `supabase start`, then (per this gotcha) restart
functions serve.
...
```bash
# Stop MinIO (data preserved in team-collections/dev/minio-data/)
docker compose -f team-collections/dev/docker-compose.yml down
...
```bash
docker compose -f team-collections/dev/docker-compose.yml down
Remove-Item -Recurse -Force team-collections/dev/minio-data   # PowerShell
# rm -rf team-collections/dev/minio-data                      # bash

supabase db reset    # drops + recreates DB, reruns all migrations, then the seed files
docker compose -f team-collections/dev/docker-compose.yml up -d
```
...
The Bloom desktop client (in the BloomDesktop repo, not this one) reads its Cloud TC
configuration from environment variables. Set these before
...
`CloudEnvironment.cs` (BloomDesktop, task 03) is the single place that reads these variables and exposes
...
2. Launch the first Bloom instance with Alice's identity:
   ...
   .\go.sh   # from a BloomDesktop checkout
   ```
...
Seeded by `supabase/seeds/tc_dev.sql`. All share the password **`BloomDev123!`**.
...
```powershell
dotnet build team-collections/dev/parity-check/ParityCheck.csproj
```

Run it against a live MinIO instance:
```powershell
# Stack must be up first (Step 4 above)
dotnet run --project team-collections/dev/parity-check/ParityCheck.csproj
```
...
## Auth settings and seed wiring

The local stack depends on two things in `supabase/config.toml`, both already in place:

- `[auth] enable_signup = true` and `[auth.email] enable_confirmations = false`: any
  email + password signs up as a confirmed user with a valid JWT, with no mail server.
  These only affect the local `supabase start` stack; hosted projects are configured in
  the Dashboard.
- `[db.seed] sql_paths` includes `./seeds/tc_dev.sql`, so `supabase db reset` loads the
  dev users above.