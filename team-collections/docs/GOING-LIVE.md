...
```powershell
# dry run first:
team-collections\aws\provision-aws.ps1 -WhatIf
team-collections\aws\provision-aws.ps1        # defaults: bloom-teams-production + bloom-teams-sandbox, us-east-1
```
...
initial migration from `supabase/schemas/tc/*.sql` (`team-collections/regen-init-migration.sh`). That is only safe
...
(document a "sandbox profile" env block in team-collections/dev/README.md). Anon keys are public by
...
Re-run `team-collections/dev/parity-check` against the sandbox bucket + real STS: sha256 checksum
...
the schema under `supabase/schemas/tc/`, the seed users in `supabase/seeds/tc_dev.sql`, and
...
1. **`bloom-core-supabase` repo** — everything that runs *in the cloud* and is not shipped inside
   ...
   `supabase/config.toml`, and the local-stack setup under `team-collections/dev/` (MinIO compose, seed
...
- *Desktop tests that require a running stack* have no obvious home: the `[Explicit]` C#
  ...
  - **(a)** Keep them in `bloom-desktop`; its CI pulls and starts the stack from
    `bloom-core-supabase` (Supabase CLI + the checked-in `team-collections/dev/` compose) to run them.
