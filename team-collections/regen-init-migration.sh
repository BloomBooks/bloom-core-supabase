...
# the init and instead add forward-only delta migrations (see team-collections/docs/CONTRACTS.md
# "Cloud functions" → declarative workflow).
#
# SAFETY: at go-live we commit a freeze marker (see team-collections/docs/GOING-LIVE.md Phase 2.2). Once
...
if [ -f "$FREEZE_MARKER" ] && [ "${ALLOW_INIT_REGEN:-}" != "1" ]; then
  {
    ...
    echo "  (see team-collections/docs/CONTRACTS.md → \"Database: declarative schema\")."
    echo
