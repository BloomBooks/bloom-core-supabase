...
for (
    const [label, lastModified] of [
        ["older than the commit window", new Date(Date.now() - 25 * 60 * 60 * 1000)],
    ...
    Deno.test(
        ...
        async () => {
            ...
            const rpcCall = calls.find((c) => c.url.includes("rpc/checkin_finish_tx"));
            assertEquals(rpcCall?.body?.p_captured, [], "a stale upload must not be captured");
            assertEquals(res.status, 409);
...
Deno.test(
    ...
    async () => {
        const { UPLOAD_COMMIT_WINDOW_MS } = await import("../_shared/tc/uploadWindows.ts");
        const s3Mock = mockClient(S3Client);
        s3Mock.on(HeadObjectCommand).resolves({
            ChecksumSHA256: hexToBase64(TX_ROW.proposed_files[0].sha256),
            LastModified: new Date(Date.now() - UPLOAD_COMMIT_WINDOW_MS + 60 * 1000),
            VersionId: "v-recent",
        });
        const calls: RecordedCall[] = [];
        ...
        assertEquals(res.status, 200);
        const rpcCall = calls.find((c) => c.url.includes("rpc/checkin_finish_tx"));
        assertEquals(rpcCall?.body?.p_captured, [{ path: "book.htm", s3VersionId: "v-recent" }]);
