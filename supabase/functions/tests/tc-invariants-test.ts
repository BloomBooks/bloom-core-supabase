...
// lifecycle config (docker-compose.yml locally; team-collections/aws/provision-aws.ps1 in production), so
...
Deno.test(
    ...
    async () => {
        const schema = await readText(
            "supabase/schemas/tc/03_tables.sql",
        );
...
Deno.test(
    ...
    async () => {
        const compose = await readText("team-collections/dev/docker-compose.yml");
        const noncurrentDaysMatch = compose.match(/--noncurrent-expire-days (\d+)/);
        assert(
            noncurrentDaysMatch,
            "could not find --noncurrent-expire-days in team-collections/dev/docker-compose.yml",
        );
        const { UPLOAD_COMMIT_WINDOW_MS } = await import(
            "../_shared/tc/uploadWindows.ts"
        );
        assert(
            UPLOAD_COMMIT_WINDOW_MS < Number(noncurrentDaysMatch[1]) * 24 * 60 * 60 * 1000,
