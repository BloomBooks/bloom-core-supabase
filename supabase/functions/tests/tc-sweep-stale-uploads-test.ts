...
const pageOf = (rows: WorklistRow[], requestBody: Record<string, unknown> | undefined) => {
...
Deno.test(
    ...
    async () => {
        ...
        s3.on(ListObjectVersionsCommand).callsFake((input: { Prefix: string }) => ({
            Versions:
                input.Prefix === lastKey
                    ? [
                        { Key: lastKey, VersionId: "garbage", IsLatest: true, LastModified: OLD },
                        { Key: lastKey, VersionId: "committed", IsLatest: false, LastModified: OLD },
                    ]
                    : [{ Key: input.Prefix, VersionId: "committed", IsLatest: true, LastModified: OLD }],
        }));
        s3.on(DeleteObjectCommand).resolves({});
        const calls: RecordedCall[] = [];

        const res = await withMockFetch(
            worklistFetch(rows, { stillStale: true, referencedVersionId: "committed" }, calls),
            () => callHandler(handler, mockRequest({}, serviceRoleToken), {}),
        );
        ...
        assertEquals(
            ...
            [
                ...
                { p_after_key: rows[SWEEP_PAGE_SIZE - 1].s3_key, p_limit: SWEEP_PAGE_SIZE },
                { p_after_key: rows[2 * SWEEP_PAGE_SIZE - 1].s3_key, p_limit: SWEEP_PAGE_SIZE },
            ],
        );
        assertEquals(
...
Deno.test(
    ...
    async () => {
        ...
        const s3 = mockClient(S3Client);
        s3.on(ListObjectVersionsCommand).callsFake((input: { Prefix: string }) => ({
            Versions: [{ Key: input.Prefix, VersionId: "committed", IsLatest: true, LastModified: OLD }],
        }));
        const calls: RecordedCall[] = [];
...
Deno.test("sweep: a page fits in one PostgREST response (max_rows 1000)", () => {
