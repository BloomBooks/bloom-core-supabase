...
function getS3UrlBase(env: Environment) {
...
export async function listPrefixContentsKeys(prefix: string, env: Environment) {
  ...
  do {
    ...
    const keys = listResponse.Contents.map((file: any) => file.Key);
...
export async function deleteFilesByPrefix(
  ...
  env: Environment,
  prefixToExclude: string | null = null
) {
  const client = getS3Client(env);
  ...
  do {
    ...
    let keys = listResponse.Contents.map((file: any) => file.Key);

    // Apparently, the API doesn't allow for getting a list with an exclusion.
    // So we do it this less efficient way.
    if (prefixToExclude) {
      keys = keys.filter((key: string) => !key.startsWith(prefixToExclude));
    }
...
export async function copyBook(
  ...
  // 1. Kick off the copies while returning to the client.
  //    When upload-finish is called by the client, it would have to verify the copy is done.
  // 2. s3 batch job - S3 would handle the status.
...
export async function processFileHashes(
  ...
  let filesNewOrModified: string[] = [];
  const filesNotChanged: string[] = [];
  ...
  do {
    ...
    if (listResponse.Contents) {
      listResponse.Contents.forEach((s3File: any) => {
...
let s3ClientProd: S3Client;
let s3ClientDev: S3Client;
let s3ClientUnitTest: S3Client;
...
function getS3Credentials(env: Environment) {
  ...
  return {
    accessKeyId:
      Deno.env.get(`BLOOM_UPLOAD_PERMISSION_MANAGER_S3_ACCESS_KEY_ID_${suffix}`) || "",
    secretAccessKey:
      Deno.env.get(`BLOOM_UPLOAD_PERMISSION_MANAGER_S3_SECRET_ACCESS_KEY_${suffix}`) || "",
  };
}
