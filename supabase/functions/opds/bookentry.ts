import { getNeglectXmlNamespaces } from "./catalog.ts";
import BloomParseServer from "../_shared/BloomParseServer.ts";
import { Environment } from "../_shared/utils.ts";

// Escapes the five XML special characters, replacing the `entities` npm package
// the Azure version used. Note a deliberate difference: `entities.encodeXML`
// also emitted every non-ASCII character (code point >= U+0080) as a numeric
// character reference, whereas this leaves them as raw UTF-8. Both are valid,
// semantically identical XML under the declared UTF-8 encoding, so every
// conformant OPDS reader decodes them the same; only a byte-for-byte diff of the
// feed differs. We intentionally keep the raw-UTF-8 form (simpler, and avoids
// surrogate-pair handling) rather than reproducing the numeric escaping.
...
export default class BookEntry {
  ...
  public static getOpdsEntryForBook(
    ...
    if (book.authors && book.authors.length > 0) {
      book.authors.map((author: string) => {
        entry += `<author><name>${encodeXML(author)}</name></author>`;
      });
    }
    ...
    const links = BookEntry.getLinkElements(book, referrerTag, env, apiBaseUrl);
  ...
  private static getLinkElements(
    ...
    const uploadBaseUrl = BloomParseServer.getUploadBaseUrl(book, apiBaseUrl);
    if (!uploadBaseUrl) {
    ...
    const harvestBaseUrl = BloomParseServer.getHarvesterBaseUrl(book, apiBaseUrl);
    const name = BloomParseServer.getBookFileName(book);
    const imageHref = BloomParseServer.getThumbnailUrl(book, apiBaseUrl);
    ...
    if (epubOnly) {
      ...
      if (book.langPointers && book.langPointers.length > 0) {
        ...
        return book.langPointers
          .map((lang: any) => makeDCElementOrEmpty("language", lang.isoCode))
          .join(" ");
      } else if (book.languages && book.languages.length > 0) {
        return book.languages
          .map((lang: string) => makeDCElementOrEmpty("language", lang))
          .join(" ");
      }
      return "";
    }
...
function makeBloomLevelElementOrEmpty(book: any): string {
  ...
  for (const prefix of priorityOrderedPrefixes) {
    ...
    if (!Number.isNaN(levelNum) && levelNum !== undefined && levelNum >= 0) {
...
export function getNumberValueFromTags(
  tags: string[],
  prefix: string
): number | undefined {
