...
export default class Catalog {
  ...
  public static async getCatalog(
    ...
    // the base for the fs links we generate, e.g. "https://api.bloomlibrary.org/v1"
    const apiBaseUrl = baseUrl.substring(0, baseUrl.indexOf("v1") + 2);

    const header = this.makeHeaderElements(apiAccount);
    ...
    const languageLinks =
      ...
      skipServerElementsForFastTesting
        ? ""
        : await Catalog.getLanguageLinks(parseServer, params, baseUrl);
              ...
              <feed  ${this.getNamespaceDeclarations()}  >
                ...
                ${
                  params.minimalnavlinks
                    ? ""
                    : this.makeOPDSDirectionLinks(params, baseUrl)
                }
                ${languageLinks}
                ${bookEntries}
              </feed>`;
  ...
  private static makeNavigationEntry(
    ...
    const ambientParameters: string = Catalog.GetParamsForHref(params, "?");
    return `<entry>
                <id>${title.split(" ").join("-")}</id>
                <title>${title}</title>
                <updated>${new Date().toISOString()}</updated>
                <link rel="subsection" href="${baseUrl}${ambientParameters}" type="application/atom+xml;profile=opds-catalog;kind=acquisition"/>
              </entry>`;
  }

  private static makeOPDSDirectionLinks(
    params: CatalogParams,
    baseUrl: string
  ): string {
    const selfUrl: string = baseUrl + this.GetParamsForHref(params, "?");
    ...
    return (
      ...
      `<link rel="up" href="${baseUrl}" ${kOpdsNavigationTypeAttribute}/>`
    );
  }
  ...
  private static makeHeaderElements(apiAccount?: ApiAccount): string {
  ...
  private static GetParamsForHref(
    ...
    const r = paramSpecs
      ...
      .filter(
        (p) =>
          (params as Record<string, unknown>)[p.name] != p.default &&
          !!(params as Record<string, unknown>)[p.name]
      ) // don't list it if it's the default value or missing
      .map(
        (p) =>
          `${p.name}=${encodeURIComponent(
            String((params as Record<string, unknown>)[p.name])
          )}`
      )
      .join("&amp;");
  ...
  private static async getLanguageLinks(
    ...
    const sortedByName = languages.sort((a: any, b: any) => {
    ...
    const consolidatedLanguages: any[] = [];
    ofInterest.forEach((l: any) => {
      let index = consolidatedLanguages.findIndex(
...
let neglectXmlNamespaces: boolean;
